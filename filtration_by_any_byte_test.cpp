...
TEST_F(HomeworkTest, test_filtration_by_any_byte) {    
    ...
    EXPECT_STREQ(filtered_by_any_byte[(filtered_by_any_byte.size()-3)].to_string().c_str(),"46.49.43.85");
    EXPECT_STREQ(filtered_by_any_byte[(filtered_by_any_byte.size()-2)].to_string().c_str(),"39.46.86.85");
    EXPECT_STREQ(filtered_by_any_byte[(filtered_by_any_byte.size()-1)].to_string().c_str(),"5.189.203.46");
}
